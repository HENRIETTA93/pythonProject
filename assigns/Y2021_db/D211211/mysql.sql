...
select
t1.PHA_NAME, t1.MR_INSPECTION_DATE,
p1.INSPECTION_COST as MR_INSPECTION_COST,
t1.SECOND_MR_INSPECTION_DATE,
p2.INSPECTION_COST as SECOND_MR_INSPECTION_COST,
p1.INSPECTION_COST-p2.INSPECTION_COST as CHANGE_IN_COST,
round((p1.INSPECTION_COST-p2.INSPECTION_COST)/p2.INSPECTION_COST*100,2)  as PERCENT_CHANGE_IN_COST
from
(
select PUBLIC_HOUSING_AGENCY_NAME as PHA_NAME,INSPECTION_DATE as MR_INSPECTION_DATE,
...
from
(
select PUBLIC_HOUSING_AGENCY_NAME, INSPECTION_DATE, sum(COST_OF_INSPECTION_IN_DOLLARS) as INSPECTION_COST
from public_housing_inspection_data
group by PUBLIC_HOUSING_AGENCY_NAME, INSPECTION_DATE
) tt1
) t1 join
(
select PUBLIC_HOUSING_AGENCY_NAME as PHA_NAME, max(INSPECTION_DATE) as MR_INSPECTION_DATE
...
group by PUBLIC_HOUSING_AGENCY_NAME
) t2 on t1.PHA_NAME=t2.PHA_NAME and t1.MR_INSPECTION_DATE=t2.MR_INSPECTION_DATE
join
(
select PUBLIC_HOUSING_AGENCY_NAME, INSPECTION_DATE, sum(COST_OF_INSPECTION_IN_DOLLARS) as INSPECTION_COST
from public_housing_inspection_data
...
on t1.MR_INSPECTION_DATE=p1.INSPECTION_DATE and t1.PHA_NAME=p1.PUBLIC_HOUSING_AGENCY_NAME
join
(
select PUBLIC_HOUSING_AGENCY_NAME, INSPECTION_DATE, sum(COST_OF_INSPECTION_IN_DOLLARS) as INSPECTION_COST
from public_housing_inspection_data
...
on t1.SECOND_MR_INSPECTION_DATE=p2.INSPECTION_DATE and t1.PHA_NAME=p2.PUBLIC_HOUSING_AGENCY_NAME;


/*

select PUBLIC_HOUSING_AGENCY_NAME as PHA_NAME,INSPECTION_DATE as MR_INSPECTION_DATE,
lag(INSPECTION_DATE,1) over(partition by PUBLIC_HOUSING_AGENCY_NAME order by INSPECTION_DATE)  as SECOND_MR_INSPECTION_DATE
from
public_housing_inspection_data;



select * from public_housing_inspection_data where PUBLIC_HOUSING_AGENCY_NAME='ALTOONA HOUSING AUTHORITY';
