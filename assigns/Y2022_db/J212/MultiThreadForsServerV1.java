...
public class MultiThreadForsServerV1 {
            ...
            try (
                ...
                while (true) {
                    ...
                    if (!allOpr.contains(inputLine)) {
                        ...
                        opr = inputLine;
                        System.out.println("Server support the operation:" + inputLine + "..[Thread ID:" + Thread.currentThread().getId() + "] wait for client's number");
                        break;
                    }
                }
                ...
                int avgCalNum = num.length / nThreads;
                int lastThreadCal = num.length - avgCalNum * (nThreads - 1);
                ...
                for (int i = 0; i < nThreads; i++) {
                    ...
                    if (i != nThreads - 1) {
                        cals[i].numbers = new int[avgCalNum];
                        for (int k = 0; k < avgCalNum; k++) {
                            cals[i].numbers[k] = num[i * avgCalNum + k];
                        }
                        cals[i].result = new int[avgCalNum];
                        cals[i].resultd = new double[avgCalNum];
                ...
                for (int i = 0; i < nThreads; i++) {
                    ...
                    if (i != nThreads - 1) {
                        while (j < avgCalNum) {
                            result[i * avgCalNum + j] = cal.result[j];
                            resultd[i * avgCalNum + j] = cal.resultd[j];
                            j++;
                        }
                        j = 0;
                    } else {
