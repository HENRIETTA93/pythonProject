...
def checkDepartType(department_type):
    if department_type not in DEPART_TYPE:
...
def checkCallType(call_type):
    if call_type not in CALL_TYPE:
        print("The call type is one of the following values: 'COMPLAINT' or 'COMPLIMENT'")
        return False
    return True


def checkCommChannel(communication_channel):
    if communication_channel not in COMMUNICATION_CHANELL:
        print("The communication channel should be one of the following values: 'PHONE', 'EMAIL', 'SOCIALMEDIA'")
        return False
    return True


def checkResolutionStatus(resolution_status):
