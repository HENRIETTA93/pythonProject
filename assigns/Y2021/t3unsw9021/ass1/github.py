...
def findMaxIndex(content, list, v, left, right):
    ...
    if maxindex - o_sign > 1 or (v.index(max_copy) - v.index(max)) > 2 \
            ...
            (left + 1 < right and content[left] == content[left + 1] and (
                    FindTopN(max_copy) == FindTopN(digitToValue(content[left + 1], list, v)) == 5)
             or (left + 2 < right and content[left] == content[left + 2] != content[left + 1] and FindTopN(
                        max_copy) == 5)) \
            or (
            left + 2 < right and FindTopN(digitToValue(content[left + 1], list, v)) == 5 and content[left] == content[
        left + 2]) \
