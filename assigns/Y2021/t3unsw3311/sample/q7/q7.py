...
try:
    ...
    query1="select * from players where name='{}'"
    cursor.execute(query1.format(player))
    player_info=cursor.fetchone()

    # print(query1.format(player))
...
order by playedon
    """
    cursor.execute(query2.format(pid))
...
group by p.id, t.country;
    """
    cursor.execute(query3.format(pid))
