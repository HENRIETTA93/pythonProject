...
try:
  ...
  if codeOf == "program":
    ...
    query_prog="select p.code, o.longname \
                from programs p join orgunits o on p.offeredby=o.id \
                where (p.code::text)='" + str(pcode)+"'"
    cursor=db.cursor()
    ...
    pid = progInfo[0]
    query_prog_rules = "select pr.program, r.name, aog.type, aog.defby, aog.definition, r.min_req,r.max_req \
    from program_rules pr join rules r on pr.rule=r.id \
    join academic_object_groups aog on aog.id=r.ao_group \
    where pr.program=" +str(pid)

    cursor.execute(query_prog_rules)
    ...
    for r in result_prog_rules:
      ...
      if aog_type=='subject' and aog_defby=='pattern':
        ...
        if len(sj_patterns)==1:
          number_rule_str=str(number_rule_str)+ " UOC of " +rule_name
          print(number_rule_str)
        elif len(sj_patterns)>1:
          number_rule_str=str(number_rule_str)+ " UOC courses from " + rule_name
          print(number_rule_str)
    ...
    query_prog="select s.code, o.longname \
                from streams s join orgunits o on s.offeredby=o.id \
                where (s.code::text)='" + str(scode)+"'"
    cursor=db.cursor()
    cursor.execute(query_prog)
    ...
    query_strm_rules = "select s.code,r.name, aog.type, aog.defby, aog.definition, r.min_req, r.max_req \
    from stream_rules sr join rules r on sr.rule=r.id \
    join academic_object_groups aog on aog.id=r.ao_group \
    join streams s on s.id=sr.stream \
    where s.code='" + str(scode)+"'"

    cursor.execute(query_strm_rules)
    ...
    for r in result_strm_rules:
      ...
      if aog_type == 'subject' and aog_defby == 'pattern':
        ...
        sj_patterns=aog_definition.split(",")
        if len(sj_patterns)==1:
          number_rule_str=str(number_rule_str)+ " UOC of " +rule_name[7:]
          print(number_rule_str)
        elif len(sj_patterns)>1:
          number_rule_str=str(number_rule_str)+ " UOC courses from " + rule_name
          print(number_rule_str)
